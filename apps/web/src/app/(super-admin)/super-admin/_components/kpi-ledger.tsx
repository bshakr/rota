...
import { deliveryRateNote, plural } from "@/lib/hq-overview";
  ...
  return (
    <ul className="mb-8 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
      <LedgerTile
        ...
        label={plural(kpis.houses_total, "house", "houses")}
      ...
      <LedgerTile
        ...
        label={`${plural(kpis.texts_last_7_days, "text", "texts")}, last 7 days`}
      ...
      <LedgerTile
        ...
        label={`${plural(kpis.covers_this_week, "cover", "covers")} this week`}
      />
      <LedgerTile
        icon={TriangleAlert}
        coin="blush"
        value={failed}
        label={`${plural(failed, "failure", "failures")}, last 7 days`}
