...
// A person in Soft Clay is a small pastel coin, round, wearing the soft clay so
// it sits on the page rather than floating flat. The tint comes from
// `avatarTint(name)` and is stable per person; the initials stay in
...
function Avatar({
  ...
  return (
    <AvatarPrimitive.Root
      ...
      data-size={size}
      className={cn(
        "group/avatar relative flex size-8 shrink-0 rounded-full shadow-xs select-none data-[size=lg]:size-10 data-[size=sm]:size-6",
        className
      )}
      {...props}
    />
