module SuperAdmin
  ...
  class GroupStats
    ...
    Row = Data.define(
      :admins, :active_members, :running_rotas, :draft_rotas, :texts, :failed_texts,
      :last_activity_at
    ) do
      ...
      def self.empty
        new(admins: 0, active_members: 0, running_rotas: 0, draft_rotas: 0, texts: 0,
          failed_texts: 0, last_activity_at: nil)
      end
    end
    ...
    def build_rows
      ...
      group_ids.index_with do |id|
        rota = rotas.fetch(id, { running: 0, draft: 0 })
        text = texts.fetch(id, { total: 0, failed: 0 })
    ...
    def rota_counts
      ...
      rosters.each_with_object({}) do |((group_id, _rota_id, active), roster_size), counts|
        row = counts[group_id] ||= { running: 0, draft: 0 }
    ...
    # Totals and failures in one grouped pass: every status is counted, and `failed` is read back
    # out of the same result rather than costing a second query.
    def text_counts
      ...
      counted.each_with_object({}) do |((group_id, status), count), totals|
        row = totals[group_id] ||= { total: 0, failed: 0 }
        row[:total] += count
