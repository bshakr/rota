...
RSpec.describe SuperAdmin::GroupStatus do
  def stats(running_rotas: 1, last_activity_at: Time.current)
    SuperAdmin::GroupStats::Row.empty.with(running_rotas: running_rotas, last_activity_at: last_activity_at)
  end

  it "is live while something is still happening" do
    expect(described_class.of(build(:group), stats)).to eq("live")
  end

  it "is quiet after thirty days of nothing" do
    expect(described_class.of(build(:group), stats(last_activity_at: 31.days.ago))).to eq("quiet")
  end

  it "is never started when no rota has a roster" do
    expect(described_class.of(build(:group), stats(running_rotas: 0))).to eq("never_started")
  end

  it "prefers never started to quiet: a house that never began is not a house that went quiet" do
    expect(described_class.of(build(:group), stats(running_rotas: 0, last_activity_at: nil))).to eq("never_started")
  end
